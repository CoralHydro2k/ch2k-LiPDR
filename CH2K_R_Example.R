# Install lipdR (if necessary; one-time process): ---------------------------------------
...
# From the lipdverse directly:
ch2k <- readLipd('https://lipdverse.org/CoralHydro2k/current_version/CoralHydro2k1_0_0.zip')
# You can also load in a local copy of the .zip file by using readLipd() interactively, or by providing a path with readLipd(path ="[your path]"):
#ch2k <- readLipd()


# Exploring the broader database: -------------------------------------------------

# Let's look at the global distribution of records in the database:
mapLipd(ch2k,
...
plotSummary(ch2k[[123]], 
            paleo.age.var = "year") # Note that paleo.data.var is not supplied here since there are multiple proxy types in the database, but you will be prompted to choose the appropriate variable.
...
# This filters out values like "d18O_annual", "SrCaUncertainty", and "year" that aren't especially useful to visualize.
ts.tib <- ts.tib %>%
...
IndOce.tib <- ts.tib %>% # Note that this only takes primary d18O and Sr/Ca time series into account
...
proxy.tib <- ts.tib %>% # Note that this only takes primary d18O and Sr/Ca time series into account
...
filter.tib <- ts.tib %>% # Note that this only takes primary d18O and Sr/Ca time series into account
...
filter2.tib <- ts.tib %>% # Note that this only takes primary d18O and Sr/Ca time series into account
